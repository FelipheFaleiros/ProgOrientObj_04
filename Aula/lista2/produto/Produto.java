package aula.lista2.produto;
public class Produto {
    //vamos encapsular as variáveis -> esconder, proteger, privar de mau uso
    private int id, qtde;
    private String descricao;
    private float preco;

    //contrutor
    public Produto(){
        
    } 
     
    public Produto(int id, int qtde, String descricao, float preco){
        this.setId(id);
        this.setQtde(qtde);
        this.setDescricao(descricao);
        this.setPreco(preco);
        
    }

    //vamos criar os métodos setters
    //método final não pode ser alterado nas classes filhas
    public final void setQtde(int qtde){
        if (qtde >= 0) {
            this.qtde = qtde;
        }
        else {
            System.out.println("Quantidade de produto não pode ser negativo");
        }
    
    }     
    public final void setPreco(float preco){
        if (preco >= 0) {
            this.preco = preco;
        }
        else {
            System.out.println("O valor do produto não pode ser negativo");
        }
    }     
    public final void setId(int id){
        if (id >= 0) {
            this.id = id;
        }
        else {
            System.out.println("O Identificado do produto não pode ser negativo");
        }
    }
    public final void setDescricao(String descricao){
        if (!descricao.isEmpty()){ //não vai validar nem vazio nem espaço / se descrição não for vazia
            this.descricao = descricao;
        }
        else {
            System.out.println("Descrição não pode estar ser vazia");
        }
    }
         
    //comprar 
    public void comprar (int x){
        this.qtde += x;
    }

    //venda 
    public void venda (int x){
        this.setQtde(this.qtde - x);
        // this.qtde -= x;
    }

    //subir
    public void subir (float x){
        this.preco += x;
    }

    //descer 
    public void descer (float x){
        this.setPreco(this.preco - x);
        // this.preco -= x;
    }

    public String mostra(){
        return String.format("Id: %d Nome:  %s Qtd: %d Preço %.2f", this.id, this.descricao, this.qtde, this.preco);
    }

}
